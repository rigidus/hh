...
;;          (w (third *block-search-squre*))
;;          (h (fourth *block-search-squre*))
;;          (path (make-pathname :name "snap.png")))
;;          (with-open-file (p path :direction :output :if-exists :supersede)
;;            (format p (x-snapshot :x x :y y :width w :height h)))))

(x-snapshot :x 250 :y 130 :width 100 :height 300)
(block test
(with-open-file (p path :direction :output :if-exists :supersede)
  (format p (x-snapshot :x x :y y :width w :height h)))
(defun dino-jump ()
