...
начала анализа)
- залочить пулл результатов - to do
- найти в нем максимальный порог белых пикселей -to do
- разлочить пулл результатов - to do
...
- сохранить туда кол-во черных точек, текущий порог белых точек и координату Y, на
  которой этот результат получен (пока что есть только кол-во черных точек и Y)
- разлочить пулл результатов
- взять следующее задание, если есть
- если нет, ждать 10 секунд (на тестовом этапе просто выходим, без ожидания)
- если тасков так и не появилось
- вернуть пулл результатов
- умереть

Главный поток на данный момент скринингом не занимается, а только ждет результат от
анализирующих потоков.


Сначала напишем функцию, которая сделает нам задания для 2х изображений.
...
А теперь все это соберем. В качестве параметра нужно передать кол-во ядер на компе.
...
#+BEGIN_SRC lisp :noweb yes
  ...
  (defun get-area-merge-results (num-of-cores)
    (let* ((lock (bt:make-lock))
      ...
      (do ((i 0 (incf i)))
                            ...
                            (let* ((cur-task (car tasks))
                              ...
                              (do ((i (- (length cur-task) 1) (- i 1)))
                                ...
                                (let ((amount (analysis
                                  ...
                                  (if amount
                                      ;; записываем его в общий пул тасков
                                      (bt:with-lock-held (lock)
        ...
        (format t "~% thread-names ~A i ~A" thread-names i)
        )
  ...
  ;;   (time
  ;;   (let ((arr1 (binarization (x-snapshot :x 440 :width 60 :height 60)))
  ;;         (arr2 (binarization (x-snapshot :x 440 :width 60 :height 60))))
  ;;     (make-task arr1 arr2)
  ;;     (make-task arr1 arr2)
...
1. У каждой пары анализируемых изображений должен быть свой пулл результатов, в противном
   случае в общем пуле уже будет не разобрать, какие результаты принадлежат какой
   картинке. Вопрос, как анализировать тогда допустимое кол-во белых точек: если каждый
   поток будет проходить по каждому пулу, это будет долго.
