...
В этой части функции мы ждем, пока анализирующий потоки закончат свою работу.
Для этого мы проходим по списку имен потоков и проверяем, живы ли они.
Если найден живой поток, то счетчик потоков инкрементируется. Таким образом,
пока жив хоть один поток, мы находимся в "режиме ожидания". Как только живых потоков
не отсается, функция возвращает конечный результат - лучшие результаты для каждой пары картинок
...
#+BEGIN_SRC lisp :noweb yes
  ...
  ;;   (time
  ;;    (let ((arr1 (binarization (load-png "~/Pictures/test4.png")))
  ;;          (arr2 (binarization (load-png "~/Pictures/test5.png"))))
  ;;      (make-task arr1 arr2)
  ;;      (make-task arr1 arr2)
  ...
  ;; (block make-task-test
  ;;   (time
  ;;    (let ((arr1 (binarization (x-snapshot :x 440 :width 20 :height 20)))
  ;;          (arr2 (binarization (x-snapshot :x 440 :width 20 :height 20))))
  ;;      (make-task arr1 arr2)
  ;;      (make-task arr1 arr2)
  ;;      (let ((res (get-area-merge-results 3)))
  ;;        (format t "~% res ~A" res)
  ;;        ))))
