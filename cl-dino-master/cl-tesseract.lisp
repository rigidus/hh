...
will raise an error."
  (format t " ~% init-tess-api() api ~A  lang ~A" api lang)
  ;; вызываем TessBaseAPIInit3:
  ;; принимает указатель на api, указатель на данные тесеракта и язык
  ;; возвращает handle->Init(datapath, language), где handle = api
  ;; видимо, так инициализируем api с папкой тесеракта (?)
  ;; если возвращен 0, все прошло успешно
  (let ((return-code (tessbaseapiinit3 api *tessdata-directory* lang)))
    (format t " ~% init-tess-api() return-code ~A" return-code)
    (case return-code
...
Returns T on success, otherwise signals error."
  (format t " ~% process-pages() api ~A truename ~A" api truename)
  (mask-sigfpe
    (let ((return-code (tessbaseapiprocesspages api truename "" 0 (null-pointer))))
      (format t " ~% process-pages() return-code ~A" return-code)
    (case return-code
      (1 t)
...
text string."
  ;; создаем api и связываем его с именем
  (with-base-api api
