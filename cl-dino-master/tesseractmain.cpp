...
int main(int argc, char **argv) {
  ...
  /* указатель на строчку, обозначающую путь (?)*/
  ...
  /* парсим аргументы*/
  /* возвращает массив с указателями на распарсенные аргументы (?)*/
  ParseArgs(argc, argv,
  ...
  bool banner = false;
  /* если outputbase (куда выводим, видимо) != nil и при этом
     outputbase это stdout.. (?)*/
  ...
  /*если не удалось инициализировать, печатаем ошибку*/
  if (init_failed) {
    fprintf(stderr, "Could not initialize tesseract.\n");
    exit(1);
  }
  /*судя по названию функции, получаем переменные из аргументов*/
  SetVariablesFromCLArgs(&api, argc, argv);

  /*если есть список языков, печатаем его*/
  if (list_langs) {
     PrintLangsList(&api);
     exit(0);
  }
  /*печатаем параметры, если есть*/
  if (print_parameters) {
     FILE* fout = stdout;
  ...
  if (pagesegmode == tesseract::PSM_AUTO_ONLY) {
    int ret_val = 0;
    /*читаем изображение и пишем их по указателю*/
    Pix* pixs = pixRead(image);
    /*если по указателю ничего нет*/
    if (!pixs) {
        /*выводим ошибку*/
    ...
    /*распознаем изображение.
      Сразу после этой функции может следовать GetUTF8Text и
      изображение будет распознано автоматически*/
    api.SetImage(pixs);
